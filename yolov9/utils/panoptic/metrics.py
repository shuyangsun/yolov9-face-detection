...
class Semantic_Metrics:
    ...
    def update(self, pred_masks, target_masks):
        ...
        for b in range(nb):
            ...
            semantic_mask = torch.flatten(onehot_mask, start_dim=1).permute(
                1, 0
    ...
    def results(self):
        ...
        miou = (
            0.0
            if (0 == len(self.iou))
            else np.sum(self.iou) / (len(self.iou) * self.nc)
        )
    ...
    def reset(self):
        ...
        self.c_intersection_counts = torch.zeros(self.nc, dtype=torch.long).to(
            self.device
        )
