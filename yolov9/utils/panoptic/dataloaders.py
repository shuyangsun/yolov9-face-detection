...
def img2stuff_paths(img_paths):
    ...
    sa, sb = (
        ...
        f"{os.sep}stuff{os.sep}",
    )  # /images/, /segmentations/ substrings
...
class LoadImagesAndLabelsAndMasks(LoadImagesAndLabels):  # for training/testing
    def __init__(
        ...
        p = Path(path)
        cache_path = (
            p.with_suffix("") if p.is_file() else Path(self.seg_files[0]).parent
        )
        ...
        try:
            ...
            cache, exists = (
                self.cache_seg_labels(cache_path, prefix),
                False,
            )  # run cache ops
        ...
        for i, (label, semantic_masks) in enumerate(
            zip(seg_labels, self.semantic_masks)
        ):
    ...
    def __getitem__(self, index):
        ...
        for cls_id, semantic_mask in zip(seg_cls, semantic_masks):
            semantic_seg_masks[cls_id] = (
                semantic_seg_masks[cls_id].logical_or(semantic_mask)
            ).int()
    ...
    def cache_seg_labels(self, path=Path("./labels_stuff.cache"), prefix=""):
        ...
        with Pool(NUM_THREADS) as pool:
            pbar = tqdm(
                pool.imap(
                    verify_image_label,
                    zip(self.im_files, self.seg_files, repeat(prefix)),
                ),
                desc=desc,
