...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        pred_distri2, pred_scores2 = torch.cat(
            [xi.view(feats2[0].shape[0], self.no, -1) for xi in feats2], 2
        ).split((self.reg_max * 4, self.nc), 1)
        pred_scores2 = pred_scores2.permute(0, 2, 1).contiguous()
        pred_distri2 = pred_distri2.permute(0, 2, 1).contiguous()

        pred_distri3, pred_scores3 = torch.cat(
            [xi.view(feats3[0].shape[0], self.no, -1) for xi in feats3], 2
        ).split((self.reg_max * 4, self.nc), 1)
        ...
        pred_bboxes2 = self.bbox_decode(
            anchor_points, pred_distri2
        )  # xyxy, (b, h*w, 4)
        pred_bboxes3 = self.bbox_decode(
            anchor_points, pred_distri3
        )  # xyxy, (b, h*w, 4)
        ...
        loss[1] = (
            0.25
            * self.BCEcls(pred_scores, target_scores.to(dtype)).sum()
            / target_scores_sum
        )  # BCE
        loss[1] += (
            0.25
            * self.BCEcls(pred_scores2, target_scores2.to(dtype)).sum()
            / target_scores_sum2
        )  # BCE
        loss[1] += (
            self.BCEcls(pred_scores3, target_scores3.to(dtype)).sum()
            / target_scores_sum3
        )  # BCE
