    ...
    for epoch in range(
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                if (opt.save_period > 0 and epoch % opt.save_period == 0) or (
                    epoch > (epochs - 2 * opt.close_mosaic)
                ):
...
def parse_opt(known=False):
    ...
    parser.add_argument(
        "--weights", type=str, default=ROOT / "yolo-pan.pt", help="initial weights path"
    )
    ...
    parser.add_argument(
        "--project", default=ROOT / "runs/train-pan", help="save to project/name"
    )
    ...
    parser.add_argument(
        "--flat-cos-lr", action="store_true", help="cosine LR scheduler"
    )
