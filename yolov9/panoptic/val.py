...
from utils.panoptic.general import (
    mask_iou,
    process_mask,
...
def run(
    ...
    for batch_i, (im, targets, paths, shapes, masks, semasks) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            preds, train_out = model(
                im
            )  # if compute_loss else (*model(im, augment=augment)[:2], None)
        ...
        # Loss
        if compute_loss:
            loss += compute_loss(train_out, targets, masks, semasks=semasks)[
                1
            ]  # box, obj, cls
        ...
        if training:
            ...
            semantic_metrics.update(
                torch.nn.functional.interpolate(
                    psemasks, size=(smh, smw), mode="bilinear", align_corners=False
                ),
                semasks,
            )
        ...
        for si, (pred, proto, psemask) in enumerate(zip(preds, protos, psemasks)):
            ...
            semantic_mask = torch.flatten(psemask, start_dim=1).permute(
                1, 0
            )  # class x h x w -> (h x w) x class
            ...
            output_masks = torch.reshape(
                output_masks.permute(1, 0), (nc, h, w)
            )  # (h x w) x class -> class x h x w
            ...
            for idx, pred_semantic_mask in enumerate(psemask):
                ...
                if 183 == category_id:
                    ...
                    pred_semantic_mask = (
                        torch.logical_xor(stuff_mask, torch.ones((h, w), device=device))
                    ).int()
                ...
                if category_id not in instances_ids:
                    rle = maskUtils.encode(
                        np.asfortranarray(pred_semantic_mask.cpu(), dtype=np.uint8)
                    )
                    rle["counts"] = rle["counts"].decode("utf-8")
            ...
            if not check_semantic_mask:
                ...
                rle = maskUtils.encode(
                    np.asfortranarray(other_mask.cpu(), dtype=np.uint8)
                )
                rle["counts"] = rle["counts"].decode("utf-8")
    ...
    LOGGER.info(
        pf
        % ("all", seen, nt.sum(), *metrics.mean_results(), *semantic_metrics.results())
    )
    if nt.sum() == 0:
        LOGGER.warning(
    ...
    if save_json and len(jdict):
        ...
        semantic_anno_json = str(
            anno_path / "annotations/stuff_val2017.json"
        )  # annotations json
        semantic_pred_json = str(
            save_dir / f"{w}_predictions_stuff.json"
        )  # predictions json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            title = (
                ...
                else " {:<5} | {:<20} | {:^6} | {:^6} ".format(
                    "class", "class name", "iou", "macc"
                )
            )
            print(title)
            for idx, (iou, macc) in enumerate(
                zip(statsClass["ious"], statsClass["maccs"])
            ):
                id = idx + 1
                ...
                content = (
                    ...
                    if (0 >= len(stuff_names))
                        ...
                        str(id),
                        str(stuff_names[getMappingIndex(id, name="stuff")]),
                        iou,
                        macc,
                    )
                )
...
def parse_opt():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default=ROOT / "data/coco128-pan.yaml",
    ...
    parser.add_argument(
        "--project", default=ROOT / "runs/val-pan", help="save results to project/name"
    )
